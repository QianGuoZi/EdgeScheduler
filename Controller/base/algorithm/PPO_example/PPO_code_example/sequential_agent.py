...
class SimpleSequentialAgent(nn.Module):
    ...
    def _calculate_state_dim(self):
        ...
        physical_summary_dim = self.max_physical_nodes * 4  # 每个物理节点：CPU总量，内存总量，CPU使用率，内存使用率
        virtual_summary_dim = self.max_virtual_nodes * 2    # 每个虚拟节点：CPU需求，内存需求
        decision_state_dim = 10  # 当前步骤、阶段、已映射节点数等
    ...
    def _encode_state(self, state: Dict) -> torch.Tensor:
        ...
        # 1. 物理网络特征编码
        physical_features = state['physical_features']  # [num_physical_nodes, 4]
        num_physical_nodes = physical_features.size(0)
        
        # 填充到最大节点数
        if num_physical_nodes < self.max_physical_nodes:
            padding = torch.zeros(self.max_physical_nodes - num_physical_nodes, 4, 
                                device=physical_features.device)
            physical_padded = torch.cat([physical_features, padding], dim=0)
        else:
            physical_padded = physical_features[:self.max_physical_nodes]
        
        features.append(physical_padded.flatten())  # [max_physical_nodes * 4]
        
        # 2. 虚拟网络特征编码
        virtual_features = state['virtual_features']  # [num_virtual_nodes, 2]
        num_virtual_nodes = virtual_features.size(0)
        
        # 填充到最大节点数
        if num_virtual_nodes < self.max_virtual_nodes:
            padding = torch.zeros(self.max_virtual_nodes - num_virtual_nodes, 2, 
        ...
        features.append(virtual_padded.flatten())  # [max_virtual_nodes * 2]
...
def test_sequential_agent():
    """测试Sequential Agent"""
    print("🧪 开始测试Sequential Agent")
    
    # 创建模拟状态
    mock_state = {
        'physical_features': torch.randn(3, 4),  # 3个物理节点
        'virtual_features': torch.randn(2, 2),   # 2个虚拟节点
        'current_step': 0,
        'mapping_phase': True,
    ...
    # 测试动作选择
    print("\n📍 测试映射阶段动作选择:")
    action, log_prob, value = agent.select_action(mock_state, temperature=1.0)
    print(f"   选择的物理节点: {action}")
    print(f"   对数概率: {log_prob:.4f}")
    print(f"   状态价值: {value:.4f}")
    
    # 测试带宽阶段
    mock_state['mapping_phase'] = False
    mock_state['current_link_index'] = 0
    print("\n🔗 测试带宽阶段动作选择:")
    action, log_prob, value = agent.select_action(mock_state, temperature=1.0)
    print(f"   选择的带宽等级: {action}")
    print(f"   对数概率: {log_prob:.4f}")
    print(f"   状态价值: {value:.4f}")
    
    print("\n✅ Sequential Agent测试完成!")
