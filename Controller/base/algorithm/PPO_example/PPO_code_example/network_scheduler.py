...
class NetworkScheduler:
    ...
    def calculate_reward(self, virtual_work: VirtualWork) -> float:
        ...
        # 4. 过度集中惩罚（防止所有节点映射到同一物理节点）
        concentration_penalty = 0
        if len(self.scheduled_nodes) > 0:
            # 统计每个物理节点映射的虚拟节点数量
            physical_node_counts = {}
            for virtual_node, physical_node in self.node_mapping.items():
                if physical_node not in physical_node_counts:
                    physical_node_counts[physical_node] = 0
                physical_node_counts[physical_node] += 1
            
            # 计算集中度
            max_mapped_nodes = max(physical_node_counts.values()) if physical_node_counts else 0
            total_virtual_nodes = len(self.scheduled_nodes)
            
            if total_virtual_nodes > 0:
                concentration_ratio = max_mapped_nodes / total_virtual_nodes
                
                # 如果超过70%的节点映射到同一物理节点，开始惩罚
                if concentration_ratio > 0.7:
                    concentration_penalty = (concentration_ratio - 0.7) / 0.3  # 归一化到0-1
                    concentration_penalty = np.clip(concentration_penalty, 0, 1)
        ...
        # 综合奖励计算
        # 权重分配：
        # - 负载均衡: 35% (CPU、内存、网络负载均衡)
        # - 带宽满足度: 30% (重点优化，包括同一节点映射奖励)
        # - 过度集中惩罚: 20% (防止过度集中)
        # - 负载过载惩罚: 10% (防止CPU、内存和带宽过载)
        # - 探索奖励: 5% (鼓励探索不同的映射方案)
        ...
        if len(self.scheduled_nodes) > 0:
            # 计算映射的多样性
            unique_physical_nodes = len(set(self.node_mapping.values()))
            total_virtual_nodes = len(self.scheduled_nodes)
            
            # 鼓励使用多个物理节点，但不过度惩罚单节点映射
            if unique_physical_nodes == 1:
                # 单节点映射给予中等奖励（因为可能有带宽优势）
                exploration_reward = 0.5
            # elif unique_physical_nodes == total_virtual_nodes:
            #     # 完全分散映射给予最高奖励
            #     exploration_reward = 1.0
            else:
                # 部分分散映射给予中等偏高的奖励
                exploration_reward = 0.5 + 0.5 * (unique_physical_nodes - 1) / (total_virtual_nodes - 1)
        
        # 计算加权平均奖励
        reward = (0.35 * load_balance_reward + 
                 0.25 * bandwidth_satisfaction +
                 0.15 * exploration_reward -
                 0.10 * concentration_penalty -
                 0.15 * overload_penalty)
    ...
    def calculate_reward_2(self, virtual_work: VirtualWork) -> float:
        """计算调度奖励（优化负载均衡和带宽满足度）"""
        if len(self.scheduled_nodes) == 0:
            return 0
        ...
        # 计算加权平均奖励
        reward = (0.4 * load_balance_reward + 
                 0.4 * bandwidth_satisfaction - 0.2 * overload_penalty)
         # 确保最终奖励在0-1范围内
        # reward = np.clip(reward, 0, 1)
        # 综合奖励计算
        # 权重分配：
        # - 负载均衡: 40% (CPU、内存、网络负载均衡)
        # - 带宽满足度: 40% (重点优化，包括同一节点映射奖励)
        # - 负载过载惩罚: 20% (防止CPU、内存和带宽过载)
        return reward
    ...
    def calculate_simple_reward(self, virtual_work: VirtualWork) -> float:
        ...
        if mapped_physical_nodes:
            ...
            # 标准差越小，负载越均衡，奖励越高
            load_balance_reward = 1.0 - (cpu_std + memory_std)  # 范围约为[0, 1]
