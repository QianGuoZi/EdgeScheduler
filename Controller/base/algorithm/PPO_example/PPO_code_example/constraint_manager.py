...
class ConstraintManager:
        ...
        for virtual_idx in range(num_virtual_nodes):
            ...
            for physical_idx in range(num_physical_nodes):
                ...
                if virtual_cpu > available_cpu or virtual_memory > available_memory:
                    print(f"！！资源不足 virtual_idx: {virtual_idx}, physical_idx: {physical_idx}, virtual_cpu: {virtual_cpu}, available_cpu: {available_cpu}, virtual_memory: {virtual_memory}, available_memory: {available_memory}")
                    constraint_matrix[virtual_idx, physical_idx] = 0.0
        # print(f"generate_node_mapping_constraints constraint_matrix: {constraint_matrix}")
    ...
    def generate_bandwidth_constraints(self,
        ...
        Args:
            virtual_edge_features: 虚拟链路特征 [num_virtual_edges, 2] (min_bandwidth, max_bandwidth)
            link_bandwidth_mappings: 每个链路的带宽映射字典 {link_key: {level: bandwidth}}
            virtual_edges: 虚拟边索引 [2, num_virtual_edges]
            expected_num_links: 期望的链路数量（可选，用于向后兼容）
            
        Returns:
            constraint_matrix: 约束矩阵 [num_virtual_edges, bandwidth_levels]
                             1.0表示可行，0.0表示不可行
        """
        num_virtual_edges = virtual_edge_features.size(0)
        
        # 初始化约束矩阵
        constraint_matrix = torch.ones(num_virtual_edges, self.bandwidth_levels, 
                                     device=virtual_edge_features.device)
        
        # 检查每个链路的带宽约束
        for link_idx in range(num_virtual_edges):
        ...
        if expected_num_links is not None and num_virtual_edges != expected_num_links:
            print(f"调整约束矩阵：从 {num_virtual_edges} 到 {expected_num_links} 个链路")
            
            if expected_num_links > num_virtual_edges:
                # 扩展约束矩阵
                new_constraint_matrix = torch.ones(expected_num_links, self.bandwidth_levels, 
                                                 device=virtual_edge_features.device)
                new_constraint_matrix[:num_virtual_edges, :] = constraint_matrix
                constraint_matrix = new_constraint_matrix
            else:
                # 截取约束矩阵
                constraint_matrix = constraint_matrix[:expected_num_links, :]
        # print(f"generate_bandwidth_constraints constraint_matrix: {constraint_matrix}")
        return constraint_matrix
