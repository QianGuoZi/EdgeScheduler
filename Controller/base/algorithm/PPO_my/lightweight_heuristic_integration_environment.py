...
class LightweightHeuristicEnvironment(SequentialNetworkSchedulerEnvironment):
    """
    轻量级启发式引导环境
    只修改奖励函数和状态表示，保持原有架构
    """
    ...
    def _get_state(self):
        ...
        # 添加轻量级启发式特征
        heuristic_features = self._compute_lightweight_heuristic_features()
        
        # 将启发式特征添加到状态中
        original_state.update(heuristic_features)
        
        return original_state
    
    def _compute_lightweight_heuristic_features(self) -> Dict:
        """
        计算轻量级启发式特征
        只添加最关键的4-6个特征，避免状态空间爆炸
        """
        features = {}
        ...
        if self.partial_mapping:
            ...
            features['load_balance_score'] = load_balance_score
        else:
            features['load_balance_score'] = 1.0  # 初始状态认为完全均衡
        
        # 2. 资源效率指标  
        if self.mapping_phase:
            resource_efficiency = self._calculate_current_resource_efficiency()
            features['resource_efficiency'] = resource_efficiency
        else:
            features['resource_efficiency'] = 0.5  # 中性值
        
        # 3. 映射进度
        if self.partial_mapping:
            mapping_progress = sum(1 for x in self.partial_mapping if x != -1) / len(self.partial_mapping)
            features['mapping_progress'] = mapping_progress
        else:
            features['mapping_progress'] = 0.0
        
        # 4. 当前阶段资源压力
        resource_pressure = self._calculate_resource_pressure()
        features['resource_pressure'] = resource_pressure
        
        # 5. 网络连通性指标（简化）
        if self.mapping_phase and self.partial_mapping:
            connectivity_score = self._calculate_simple_connectivity()
            features['connectivity_score'] = connectivity_score
        else:
            features['connectivity_score'] = 1.0
...
def create_lightweight_heuristic_environment(**kwargs):
    """创建轻量级启发式环境"""
    return LightweightHeuristicEnvironment(**kwargs)
...
def test_lightweight_integration():
    ...
    # 创建环境
    env = LightweightHeuristicEnvironment(
