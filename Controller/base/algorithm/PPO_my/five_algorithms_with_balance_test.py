...
def evaluate_five_algorithms(ppo_agent: SimpleSequentialAgent,
                              ppo_env_config: Dict,
    ...
    for num_tasks in task_range:
        ...
        for ep in range(episodes_per_task_count):
            ...
            env_flexi = _make_ppo_env_with_num_tasks(ppo_env_config, num_tasks, ep_seed, ppo_env_type)
            env_smart = _make_ppo_env_with_num_tasks(ppo_env_config, num_tasks, ep_seed, ppo_env_type)
            env_rnd = _make_ppo_env_with_num_tasks(ppo_env_config, num_tasks, ep_seed, ppo_env_type)
        ...
        node_result = {
            ...
            'comparisons': {
                'ppo_vs_flexitask': {
                    'reward_improvement': ppo_reward_mean - flexi_reward_mean,
                    'success_rate_improvement': ppo_sr - flexi_sr
                },
                'ppo_vs_smart': {
                    'reward_improvement': ppo_reward_mean - smart_reward_mean,
                    'success_rate_improvement': ppo_sr - smart_sr
                },
                'ppo_balance_vs_flexitask': {
                    'reward_improvement': balance_reward_mean - flexi_reward_mean,
                    'success_rate_improvement': balance_sr - flexi_sr
                },
                'ppo_balance_vs_smart': {
                    'reward_improvement': balance_reward_mean - smart_reward_mean,
                    'success_rate_improvement': balance_sr - smart_sr
                },
                'ppo_vs_ppo_balance': {
                ...
                'flexitask_vs_smart': {
                    'reward_improvement': flexi_reward_mean - smart_reward_mean,
                    'success_rate_improvement': flexi_sr - smart_sr
                }
            }
        }
...
def _generate_summary(results: Dict) -> Dict:
    ...
    flexi_rewards = [detailed[n]['flexitask']['avg_reward'] for n in task_range]
    smart_rewards = [detailed[n]['smart']['avg_reward'] for n in task_range]
    ...
    flexi_success_rates = [detailed[n]['flexitask']['success_rate'] for n in task_range]
    smart_success_rates = [detailed[n]['smart']['success_rate'] for n in task_range]
    ...
    summary = {
        ...
        'win_rates': {
            'ppo_vs_flexitask': 0,
            'ppo_vs_smart': 0,
            'ppo_balance_vs_flexitask': 0,
            'ppo_balance_vs_smart': 0,
            'ppo_vs_ppo_balance': 0,
            'ppo_vs_random': 0,
            'ppo_balance_vs_random': 0,
            'flexitask_vs_smart': 0
        }
    }
    
    # 计算胜率
    ppo_beats_flexi = sum(1 for n in task_range if detailed[n]['ppo']['avg_reward'] > detailed[n]['flexitask']['avg_reward'])
    ppo_beats_smart = sum(1 for n in task_range if detailed[n]['ppo']['avg_reward'] > detailed[n]['smart']['avg_reward'])
    balance_beats_flexi = sum(1 for n in task_range if detailed[n]['ppo_balance']['avg_reward'] > detailed[n]['flexitask']['avg_reward'])
    balance_beats_smart = sum(1 for n in task_range if detailed[n]['ppo_balance']['avg_reward'] > detailed[n]['smart']['avg_reward'])
    ...
    flexi_beats_smart = sum(1 for n in task_range if detailed[n]['flexitask']['avg_reward'] > detailed[n]['smart']['avg_reward'])
    
    summary['win_rates']['ppo_vs_flexitask'] = ppo_beats_flexi / len(task_range)
    summary['win_rates']['ppo_vs_smart'] = ppo_beats_smart / len(task_range)
    summary['win_rates']['ppo_balance_vs_flexitask'] = balance_beats_flexi / len(task_range)
    summary['win_rates']['ppo_balance_vs_smart'] = balance_beats_smart / len(task_range)
    ...
    summary['win_rates']['flexitask_vs_smart'] = flexi_beats_smart / len(task_range)
    ...
    avg_performance = [
        ...
        ('FlexiTask', summary['overall_performance']['flexitask']['avg_reward'], summary['overall_performance']['flexitask']['avg_success_rate']),
        ('Smart', summary['overall_performance']['smart']['avg_reward'], summary['overall_performance']['smart']['avg_success_rate']),
...
def _save_results(results: Dict, out_dir: str = 'test_results') -> str:
    ...
    path = os.path.join(out_dir, f'five_algorithms_with_balance_{ts}.json')
...
def _create_visualization(results: Dict, out_dir: str = 'test_results') -> List[str]:
    ...
    flexi_success_rates = [detailed[n]['flexitask']['success_rate'] * 100 for n in task_range]
    smart_success_rates = [detailed[n]['smart']['success_rate'] * 100 for n in task_range]
    ...
    flexi_reward_stds = [detailed[n]['flexitask']['std_reward'] for n in task_range]
    smart_reward_stds = [detailed[n]['smart']['std_reward'] for n in task_range]
    ...
    flexi_l_values = [detailed[n]['flexitask']['load_balance']['mean'] for n in task_range]
    smart_l_values = [detailed[n]['smart']['load_balance']['mean'] for n in task_range]
    ...
    flexi_l_stds = [detailed[n]['flexitask']['load_balance']['std'] for n in task_range]
    smart_l_stds = [detailed[n]['smart']['load_balance']['std'] for n in task_range]
    ...
    flexi_dbw_values = [detailed[n]['flexitask']['bandwidth_satisfaction']['mean'] for n in task_range]
    smart_dbw_values = [detailed[n]['smart']['bandwidth_satisfaction']['mean'] for n in task_range]
    ...
    flexi_dbw_stds = [detailed[n]['flexitask']['bandwidth_satisfaction']['std'] for n in task_range]
    smart_dbw_stds = [detailed[n]['smart']['bandwidth_satisfaction']['std'] for n in task_range]
    ...
    flexi_composite_values = [_calculate_composite_score(flexi_l_values[i], flexi_dbw_values[i]) for i in range(len(task_range))]
    smart_composite_values = [_calculate_composite_score(smart_l_values[i], smart_dbw_values[i]) for i in range(len(task_range))]
    ...
    flexi_composite_stds = [_calculate_composite_std(flexi_l_stds[i], flexi_dbw_stds[i]) for i in range(len(task_range))]
    smart_composite_stds = [_calculate_composite_std(smart_l_stds[i], smart_dbw_stds[i]) for i in range(len(task_range))]
    ...
    ax1.errorbar(task_range, flexi_rewards, yerr=flexi_reward_stds,
                label='FlexiTask Heuristic', marker='^', linewidth=2, capsize=5, color=colors[2])
    ax1.errorbar(task_range, smart_rewards, yerr=smart_reward_stds,
                label='Smart Heuristic', marker='v', linewidth=2, capsize=5, color=colors[3])
    ax1.errorbar(task_range, rnd_rewards, yerr=rnd_reward_stds,
                label='Random', marker='d', linewidth=2, capsize=5, color=colors[4])
    ax1.set_xlabel('Number of Tasks')
    ax1.set_ylabel('Average Reward')
    ax1.set_title('Average Reward Comparison')
    ax1.set_xticks(task_range)
    ax1.legend()
    ...
    ax2.plot(task_range, flexi_success_rates, label='FlexiTask Heuristic', 
            marker='^', linewidth=2, color=colors[2])
    ax2.plot(task_range, smart_success_rates, label='Smart Heuristic', 
            marker='v', linewidth=2, color=colors[3])
    ax2.plot(task_range, rnd_success_rates, label='Random', 
            marker='d', linewidth=2, color=colors[4])
    ax2.set_xlabel('Number of Tasks')
    ax2.set_ylabel('Success Rate (%)')
    ax2.set_title('Success Rate Comparison')
    ax2.set_xticks(task_range)
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    performance_comparison_path = os.path.join(out_dir, f'five_algorithms_with_balance_performance_{ts}.png')
    ...
    _plot_with_nan_handling(ax1, task_range, flexi_l_values, flexi_l_stds,
                           'FlexiTask Heuristic', '^', colors[2])
    _plot_with_nan_handling(ax1, task_range, smart_l_values, smart_l_stds,
    ...
    _plot_with_nan_handling(ax2, task_range, balance_dbw_values, balance_dbw_stds,
                           'PPO_balance', 's', colors[1])
    _plot_with_nan_handling(ax2, task_range, flexi_dbw_values, flexi_dbw_stds,
                           'FlexiTask Heuristic', '^', colors[2])
    _plot_with_nan_handling(ax2, task_range, smart_dbw_values, smart_dbw_stds,
                           'Smart Heuristic', 'v', colors[3])
    ...
    _plot_with_nan_handling(ax3, task_range, balance_composite_values, balance_composite_stds,
                           'PPO_balance', 's', colors[1])
    _plot_with_nan_handling(ax3, task_range, flexi_composite_values, flexi_composite_stds,
                           'FlexiTask Heuristic', '^', colors[2])
    _plot_with_nan_handling(ax3, task_range, smart_composite_values, smart_composite_stds,
                           'Smart Heuristic', 'v', colors[3])
    ...
    ax3.set_xticks(task_range)
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    
    plt.tight_layout()
    lb_bw_comparison_path = os.path.join(out_dir, f'five_algorithms_with_balance_lb_bw_{ts}.png')
...
def print_detailed_results(results: Dict):
    ...
    print(f"\n📋 详细结果:")
    print("-"*160)
    header = f"{'任务数':<6} {'PPO奖励':<10} {'PPO_bal奖励':<12} {'FlexiTask奖励':<14} {'Smart奖励':<12} {'Random奖励':<12} {'PPO成功率':<10} {'PPO_bal成功率':<12} {'FlexiTask成功率':<14} {'Smart成功率':<12} {'Random成功率':<12}"
    print(header)
    print("-"*160)
    ...
    print(f"\n🏆 胜率统计:")
    print(f"PPO vs FlexiTask: {win_rates['ppo_vs_flexitask']:.1%}")
    print(f"PPO vs Smart: {win_rates['ppo_vs_smart']:.1%}")
    print(f"PPO_balance vs FlexiTask: {win_rates['ppo_balance_vs_flexitask']:.1%}")
    print(f"PPO_balance vs Smart: {win_rates['ppo_balance_vs_smart']:.1%}")
    ...
    print(f"FlexiTask vs Smart: {win_rates['flexitask_vs_smart']:.1%}")
...
def main():
    print("🧪 五算法性能对比测试 (PPO vs PPO_balance vs FlexiTask vs Smart vs Random)")
    print("="*80)
    ...
    try:
        ...
        # 执行评测
        results = evaluate_five_algorithms(
            ppo_agent=ppo_agent,
        ...
        elif best_algorithm == "FlexiTask":
            print("🥇 FlexiTask算法表现最佳!")
        elif best_algorithm == "Smart":
            print("🥇 Smart算法表现最佳!")
        else:
            print("⚠️ 所有算法表现都需要改进")
