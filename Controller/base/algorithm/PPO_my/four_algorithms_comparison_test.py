...
def _find_mapping_checkpoint_directories() -> List[str]:
    """查找PPO_mapping的结果目录"""
    mapping_results_dir = "../PPO_mapping/mapping_results"
    if not os.path.isdir(mapping_results_dir):
        raise FileNotFoundError(f"Mapping results directory not found: {mapping_results_dir}")
    
    directories = []
    for item in os.listdir(mapping_results_dir):
        item_path = os.path.join(mapping_results_dir, item)
        if os.path.isdir(item_path) and item.startswith("mapping_ppo_"):
            checkpoints_dir = os.path.join(item_path, "checkpoints")
            if os.path.isdir(checkpoints_dir):
                directories.append(item_path)
    
    if not directories:
        raise FileNotFoundError(f"No mapping checkpoint directories found in {mapping_results_dir}")
    
    directories.sort()
    return directories


def _find_latest_mapping_checkpoint(mapping_result_dir: str) -> str:
    """查找最新的mapping模型文件"""
    checkpoints_dir = os.path.join(mapping_result_dir, "checkpoints")
    if not os.path.isdir(checkpoints_dir):
        raise FileNotFoundError(f"Checkpoints directory not found: {checkpoints_dir}")
...
def _select_mapping_checkpoint() -> Tuple[str, Dict]:
    """交互式选择PPO_mapping checkpoint目录"""
    print("🔍 查找可用的PPO_mapping checkpoint目录...")
    
    try:
        directories = _find_mapping_checkpoint_directories()
        
        print(f"\n📁 找到 {len(directories)} 个PPO_mapping checkpoint目录:")
        print("-" * 60)
        ...
        while True:
            try:
                choice = input(f"请选择PPO_mapping目录 (1-{len(directories)}) 或输入 'q' 退出: ").strip()
                ...
                if 0 <= choice_idx < len(directories):
                    selected_dir = directories[choice_idx]
                    selected_name = os.path.basename(selected_dir)
                    print(f"✅ 选择了PPO_mapping目录: {selected_name}")
                    ...
                    model_path = _find_latest_mapping_checkpoint(selected_dir)
        ...
        print(f"❌ 查找PPO_mapping checkpoint目录失败: {e}")
        return None, None
...
def _load_mapping_agent_and_configs(model_path: str, config: Dict) -> Tuple[MappingAgent, Dict, Dict]:
    """加载PPO_mapping Agent与配置"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"🔄 加载PPO_mapping模型: {model_path}")
    
    env_config = config.get('env_config', {})
    agent_config = config.get('agent_config', {})
    
    # 创建Agent并载入权重
    agent = MappingAgent(**agent_config)
    ...
    print("✅ PPO_mapping Agent与配置加载完成")
    return agent, env_config, agent_config
...
def _make_mapping_env_with_virtual_nodes(env_config: Dict, num_virtual_nodes: int, seed: int) -> MappingNetworkSchedulerEnvironment:
    """基于给定配置、虚拟节点数和种子构建PPO_mapping环境。"""
    cfg = dict(env_config)
    cfg['seed'] = seed
    ...
    env = MappingNetworkSchedulerEnvironment(**cfg)
    return env
...
def _select_mapping_action_greedy(env: MappingNetworkSchedulerEnvironment,
                                  agent: MappingAgent,
                                  state: Dict) -> int:
    """PPO_mapping贪心策略选择映射动作"""
    agent.eval()
    with torch.no_grad():
        action_logits, _, _ = agent.forward(state)
        action_logits = action_logits.detach().cpu().numpy()
    
    num_physical_nodes = state.get('num_physical_nodes', env.num_physical_nodes)
    candidates = np.argsort(-action_logits[:num_physical_nodes])  # 降序
    
    current_virtual_node = state.get('current_virtual_node', 0)
    # 逐个尝试候选动作，选择第一个有效动作
    for a in candidates:
        is_valid, _ = env._validate_mapping_action(current_virtual_node, int(a))
        if is_valid:
            return int(a)
...
def _run_mapping_episode(env: MappingNetworkSchedulerEnvironment,
                         agent: MappingAgent,
                         max_steps: int = 50) -> Tuple[float, bool, Dict]:
    """运行PPO_mapping算法的Episode"""
    state = env.reset()
    
    # 尝试为PPO_mapping环境也集成原始奖励计算器
    if hasattr(env, 'network_scheduler') and env.network_scheduler is not None:
        try:
            from original_reward import integrate_with_network_scheduler
            integrate_with_network_scheduler(env.network_scheduler)
        except Exception as e:
            print(f"Warning: Failed to integrate original reward calculator for PPO_mapping: {e}")
    ...
    episode_info = {
        'num_virtual_nodes': state.get('num_virtual_nodes', 0),
        'num_virtual_links': state.get('num_virtual_links', 0),
        'algorithm': 'PPO_mapping',
        'steps': 0
    }

    while not done and steps < max_steps:
        if state.get('mapping_phase', True):
            # 映射阶段：使用PPO_mapping agent
            action = _select_mapping_action_greedy(env, agent, state)
        else:
            # 带宽阶段：PPO_mapping环境会自动处理带宽分配
            # 不需要额外的动作，直接step将完成带宽分配
            action = 0
    ...
    # 成功判定 - 检查episode是否成功完成
    # PPO_mapping环境如果成功完成，done会为True且reward为正
    success = bool(done and total_reward > 0)
    
    # 计算L和D_BW指标 - PPO_mapping环境可能没有这些指标
    load_balance_degree = float('nan')
    bandwidth_satisfaction = float('nan')
    ...
    if (hasattr(env, 'network_scheduler') and env.network_scheduler is not None and 
        ...
        try:
            ...
            print(f"Warning: Failed to calculate L and D_BW for PPO_mapping: {e}")
...
def _run_random_episode(env,
    ...
    if hasattr(env, 'network_scheduler') and env.network_scheduler is not None:
        try:
            ...
            print(f"Warning: Failed to integrate original reward calculator for Random: {e}")
    ...
    while not done and steps < max_steps:
        ...
        if state.get('mapping_phase', True):
            ...
            if hasattr(env, 'bandwidth_levels'):
                ...
                # PPO_mapping环境没有带宽等级概念，自动处理带宽分配
                action = 0
...
def evaluate_four_algorithms(ppo_agent: SimpleSequentialAgent,
                              ...
                              mapping_env_config: Dict,
                              virtual_nodes_range: List[int] = [3, 4, 5, 6],
    ...
    print(f"🎯 算法类型: PPO ({ppo_env_type}), PPO_mapping, Heuristic, Random")
    ...
    results = {
        'algorithms': ['PPO', 'PPO_mapping', 'Heuristic', 'Random'],
    ...
    for num_virtual_nodes in virtual_nodes_range:
        ...
        for ep in range(episodes_per_node_count):
            ...
            # 创建四个相同配置的环境
            env_ppo = _make_ppo_env_with_virtual_nodes(ppo_env_config, num_virtual_nodes, ep_seed, ppo_env_type)
            env_mapping = _make_mapping_env_with_virtual_nodes(mapping_env_config, num_virtual_nodes, ep_seed)
            env_heu = _make_ppo_env_with_virtual_nodes(ppo_env_config, num_virtual_nodes, ep_seed, ppo_env_type)
            env_rnd_ppo = _make_ppo_env_with_virtual_nodes(ppo_env_config, num_virtual_nodes, ep_seed, ppo_env_type)
            ...
            # 运行PPO_mapping
            r_mapping, s_mapping, info_mapping = _run_mapping_episode(env_mapping, mapping_agent)
            mapping_rewards.append(r_mapping)
            mapping_success.append(1.0 if s_mapping else 0.0)
            mapping_steps.append(info_mapping['steps'])
            mapping_l_values.append(info_mapping.get('load_balance_degree', float('nan')))
            mapping_dbw_values.append(info_mapping.get('bandwidth_satisfaction', float('nan')))
        ...
        mapping_reward_mean, mapping_reward_std, mapping_reward_min, mapping_reward_max = _stats(mapping_rewards)
        ...
        mapping_l_mean, mapping_l_std, mapping_l_min, mapping_l_max = _stats_with_nan(mapping_l_values)
        ...
        mapping_dbw_mean, mapping_dbw_std, mapping_dbw_min, mapping_dbw_max = _stats_with_nan(mapping_dbw_values)
        ...
        node_result = {
            ...
            'ppo_mapping': {
                'avg_reward': mapping_reward_mean,
                'std_reward': mapping_reward_std,
                'min_reward': mapping_reward_min,
                'max_reward': mapping_reward_max,
                'success_rate': mapping_sr,
                'avg_steps': mapping_avg_steps,
                'load_balance': {
                    'mean': mapping_l_mean,
                    'std': mapping_l_std,
                    'min': mapping_l_min,
            ...
            'comparisons': {
                ...
                'ppo_mapping_vs_heuristic': {
                    'reward_improvement': mapping_reward_mean - heu_reward_mean,
                    'success_rate_improvement': mapping_sr - heu_sr
                },
                'ppo_vs_ppo_mapping': {
                    'reward_improvement': ppo_reward_mean - mapping_reward_mean,
                    'success_rate_improvement': ppo_sr - mapping_sr
                },
                'ppo_vs_random': {
                    'reward_improvement': ppo_reward_mean - rnd_reward_mean,
                    'success_rate_improvement': ppo_sr - rnd_sr
                },
                'ppo_mapping_vs_random': {
                    'reward_improvement': mapping_reward_mean - rnd_reward_mean,
                    'success_rate_improvement': mapping_sr - rnd_sr
                }
            }
        }
        ...
        print(f"   PPO_mapping -> 奖励: {mapping_reward_mean:.3f}±{mapping_reward_std:.3f}, 成功率: {mapping_sr:.1%}, 平均步数: {mapping_avg_steps:.1f}")
        print(f"                -> L: {mapping_l_mean:.4f}±{mapping_l_std:.4f}, D_BW: {mapping_dbw_mean:.4f}±{mapping_dbw_std:.4f}")
...
def _generate_summary(results: Dict) -> Dict:
    ...
    mapping_rewards = [detailed[n]['ppo_mapping']['avg_reward'] for n in virtual_nodes_range]
    ...
    mapping_success_rates = [detailed[n]['ppo_mapping']['success_rate'] for n in virtual_nodes_range]
    ...
    summary = {
        'overall_performance': {
            ...
            'ppo_mapping': {
                'avg_reward': float(np.mean(mapping_rewards)),
                'avg_success_rate': float(np.mean(mapping_success_rates))
            },
            'heuristic': {
        ...
        'win_rates': {
            'ppo_vs_heuristic': 0,
            'ppo_mapping_vs_heuristic': 0,
            'ppo_vs_ppo_mapping': 0,
            'ppo_vs_random': 0,
            'ppo_mapping_vs_random': 0
        }
    }
    
    # 计算胜率
    ppo_beats_heu = sum(1 for n in virtual_nodes_range if detailed[n]['ppo']['avg_reward'] > detailed[n]['heuristic']['avg_reward'])
    mapping_beats_heu = sum(1 for n in virtual_nodes_range if detailed[n]['ppo_mapping']['avg_reward'] > detailed[n]['heuristic']['avg_reward'])
    ppo_beats_mapping = sum(1 for n in virtual_nodes_range if detailed[n]['ppo']['avg_reward'] > detailed[n]['ppo_mapping']['avg_reward'])
    ppo_beats_rnd = sum(1 for n in virtual_nodes_range if detailed[n]['ppo']['avg_reward'] > detailed[n]['random']['avg_reward'])
    mapping_beats_rnd = sum(1 for n in virtual_nodes_range if detailed[n]['ppo_mapping']['avg_reward'] > detailed[n]['random']['avg_reward'])
    
    summary['win_rates']['ppo_vs_heuristic'] = ppo_beats_heu / len(virtual_nodes_range)
    summary['win_rates']['ppo_mapping_vs_heuristic'] = mapping_beats_heu / len(virtual_nodes_range)
    summary['win_rates']['ppo_vs_ppo_mapping'] = ppo_beats_mapping / len(virtual_nodes_range)
    summary['win_rates']['ppo_vs_random'] = ppo_beats_rnd / len(virtual_nodes_range)
    summary['win_rates']['ppo_mapping_vs_random'] = mapping_beats_rnd / len(virtual_nodes_range)
    
    # 排名
    avg_performance = [
        ('PPO', summary['overall_performance']['ppo']['avg_reward'], summary['overall_performance']['ppo']['avg_success_rate']),
        ('PPO_mapping', summary['overall_performance']['ppo_mapping']['avg_reward'], summary['overall_performance']['ppo_mapping']['avg_success_rate']),
...
def _save_results(results: Dict, out_dir: str = 'test_results') -> str:
    ...
    path = os.path.join(out_dir, f'four_algorithms_comparison_{ts}.json')
...
def _create_visualization(results: Dict, out_dir: str = 'test_results') -> List[str]:
    ...
    mapping_success_rates = [detailed[n]['ppo_mapping']['success_rate'] * 100 for n in virtual_nodes_range]
    ...
    mapping_reward_stds = [detailed[n]['ppo_mapping']['std_reward'] for n in virtual_nodes_range]
    ...
    mapping_l_values = [detailed[n]['ppo_mapping']['load_balance']['mean'] for n in virtual_nodes_range]
    ...
    mapping_l_stds = [detailed[n]['ppo_mapping']['load_balance']['std'] for n in virtual_nodes_range]
    ...
    mapping_dbw_values = [detailed[n]['ppo_mapping']['bandwidth_satisfaction']['mean'] for n in virtual_nodes_range]
    ...
    mapping_dbw_stds = [detailed[n]['ppo_mapping']['bandwidth_satisfaction']['std'] for n in virtual_nodes_range]
    ...
    ppo_composite_values = [_calculate_composite_score(ppo_l_values[i], ppo_dbw_values[i]) for i in range(len(virtual_nodes_range))]
    mapping_composite_values = [_calculate_composite_score(mapping_l_values[i], mapping_dbw_values[i]) for i in range(len(virtual_nodes_range))]
    heu_composite_values = [_calculate_composite_score(heu_l_values[i], heu_dbw_values[i]) for i in range(len(virtual_nodes_range))]
    rnd_composite_values = [_calculate_composite_score(rnd_l_values[i], rnd_dbw_values[i]) for i in range(len(virtual_nodes_range))]
    ...
    ppo_composite_stds = [_calculate_composite_std(ppo_l_stds[i], ppo_dbw_stds[i]) for i in range(len(virtual_nodes_range))]
    mapping_composite_stds = [_calculate_composite_std(mapping_l_stds[i], mapping_dbw_stds[i]) for i in range(len(virtual_nodes_range))]
    heu_composite_stds = [_calculate_composite_std(heu_l_stds[i], heu_dbw_stds[i]) for i in range(len(virtual_nodes_range))]
    rnd_composite_stds = [_calculate_composite_std(rnd_l_stds[i], rnd_dbw_stds[i]) for i in range(len(virtual_nodes_range))]
    
    saved_files = []
    
    # 设置图表样式
    plt.style.use('seaborn-v0_8')
    colors = ['#2E86AB', '#A23B72', '#F18F01', '#7B2CBF']  # PPO, PPO_mapping, Heuristic, Random
    ...
    ax1.errorbar(virtual_nodes_range, mapping_rewards, yerr=mapping_reward_stds,
                label='PPO_mapping', marker='s', linewidth=2, capsize=5, color=colors[1])
    ...
    # 成功率对比
    ax2.plot(virtual_nodes_range, ppo_success_rates, label=f'PPO ({ppo_env_type})', 
            marker='o', linewidth=2, color=colors[0])
    ax2.plot(virtual_nodes_range, mapping_success_rates, label='PPO_mapping', 
    ...
    plt.tight_layout()
    performance_comparison_path = os.path.join(out_dir, f'four_algorithms_performance_{ts}.png')
    ...
    _plot_with_nan_handling(ax1, virtual_nodes_range, mapping_l_values, mapping_l_stds,
                           'PPO_mapping', 's', colors[1])
    ...
    _plot_with_nan_handling(ax2, virtual_nodes_range, ppo_dbw_values, ppo_dbw_stds,
                           f'PPO ({ppo_env_type})', 'o', colors[0])
    _plot_with_nan_handling(ax2, virtual_nodes_range, mapping_dbw_values, mapping_dbw_stds,
                           'PPO_mapping', 's', colors[1])
    ...
    # 综合指标对比 (越大越好)
    _plot_with_nan_handling(ax3, virtual_nodes_range, ppo_composite_values, ppo_composite_stds,
                           f'PPO ({ppo_env_type})', 'o', colors[0])
    _plot_with_nan_handling(ax3, virtual_nodes_range, mapping_composite_values, mapping_composite_stds,
                           'PPO_mapping', 's', colors[1])
    _plot_with_nan_handling(ax3, virtual_nodes_range, heu_composite_values, heu_composite_stds,
                           f'{heuristic_type} Heuristic', '^', colors[2])
    _plot_with_nan_handling(ax3, virtual_nodes_range, rnd_composite_values, rnd_composite_stds,
                           'Random', 'd', colors[3])
    ax3.set_xlabel('Virtual Nodes Number')
    ...
    plt.tight_layout()
    lb_bw_comparison_path = os.path.join(out_dir, f'four_algorithms_lb_bw_{ts}.png')
...
def print_detailed_results(results: Dict):
    ...
    ppo_env_type = results.get('ppo_env_type', 'Sequential')
    print(f"🔧 测试配置:")
    print(f"   算法类型: PPO ({ppo_env_type}), PPO_mapping, {heuristic_type} Heuristic, Random")
    ...
    print(f"\n📋 详细结果:")
    print("-"*140)
    header = f"{'节点数':<6} {'PPO奖励':<10} {'PPO_map奖励':<12} {'启发式奖励':<12} {'随机奖励':<10} {'PPO成功率':<10} {'PPO_map成功率':<12} {'启发式成功率':<12} {'随机成功率':<10}"
    print(header)
    print("-"*140)
    ...
    for num_nodes in results['virtual_nodes_range']:
        ...
        mapping_l_str = _format_metric(mapping['load_balance']['mean'], mapping['load_balance']['std'])
        ...
        mapping_dbw_str = _format_metric(mapping['bandwidth_satisfaction']['mean'], mapping['bandwidth_satisfaction']['std'])
        ...
        l_row = f"{'L值':<6} {ppo_l_str:<10} {mapping_l_str:<12} {heu_l_str:<12} {rnd_l_str:<10}"
        dbw_row = f"{'D_BW值':<6} {ppo_dbw_str:<10} {mapping_dbw_str:<12} {heu_dbw_str:<12} {rnd_dbw_str:<10}"
    ...
    print(f"PPO_mapping平均性能 -> 奖励: {overall['ppo_mapping']['avg_reward']:.3f}, 成功率: {overall['ppo_mapping']['avg_success_rate']:.1%}")
    ...
    print(f"PPO_mapping vs {heuristic_type}启发式: {win_rates['ppo_mapping_vs_heuristic']:.1%}")
    print(f"PPO vs PPO_mapping: {win_rates['ppo_vs_ppo_mapping']:.1%}")
    print(f"PPO vs Random: {win_rates['ppo_vs_random']:.1%}")
    print(f"PPO_mapping vs Random: {win_rates['ppo_mapping_vs_random']:.1%}")
...
def main():
    print("🧪 四算法性能对比测试 (PPO vs PPO_mapping vs Heuristic vs Random)")
    print("="*80)
    ...
    try:
        ...
        print("\n步骤3: 选择PPO_mapping模型")
        mapping_ckpt, mapping_config = _select_mapping_checkpoint()
        if mapping_ckpt is None:
            print("❌ 未选择PPO_mapping checkpoint，程序退出")
            return
        
        # 加载模型
        print(f"\n📂 加载模型...")
        ppo_agent, ppo_env_config, _ = _load_ppo_agent_and_configs(ppo_ckpt)
        mapping_agent, mapping_env_config, _ = _load_mapping_agent_and_configs(mapping_ckpt, mapping_config)

        # 测试参数
        virtual_nodes_range = [3, 4, 5, 6, 7, 8]  # 测试3-6个虚拟节点
        episodes_per_node_count = 30  # 每个节点数量测试30次
        base_seed = 42
        temperature = 0.05  # 低温度，更贪心
        heuristic_type = "flexitask"  # 使用天真启发式算法 naive, moderate, smart, flexitask
        ...
        results = evaluate_four_algorithms(
            ...
            mapping_env_config=mapping_env_config,
            virtual_nodes_range=virtual_nodes_range,
            episodes_per_node_count=episodes_per_node_count,
            base_seed=base_seed,
            temperature=temperature,
        ...
        if best_algorithm == "PPO":
            print("🥇 PPO算法表现最佳!")
        elif best_algorithm == "PPO_mapping":
            print("🥇 PPO_mapping算法表现最佳!")
        ...
        print(f"   PPO_mapping vs 启发式: {win_rates['ppo_mapping_vs_heuristic']:.1%}")
