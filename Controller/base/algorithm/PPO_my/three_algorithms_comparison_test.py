...
def _select_checkpoint_directory() -> str:
    """交互式选择checkpoint目录"""
    print("🔍 查找可用的checkpoint目录...")
    
    try:
        directories = _list_checkpoint_directories()
        
        print(f"\n📁 找到 {len(directories)} 个checkpoint目录:")
        print("-" * 60)
        ...
        while True:
            try:
                choice = input(f"请选择checkpoint目录 (1-{len(directories)}) 或输入 'q' 退出: ").strip()
...
def _create_visualization(results: Dict, out_dir: str = 'test_results') -> List[str]:
    ...
    # 计算综合指标 = 0.6*L + 0.4*D_BW (注意：这里L越小越好，D_BW越大越好，所以我们调整为 0.6*(1-L) + 0.4*D_BW 来保持越大越好的一致性)
    def _calculate_composite_score(l_val, dbw_val, l_weight=0.6, dbw_weight=0.4):
        """计算综合指标，处理NaN值"""
    ...
    def _calculate_composite_std(l_std, dbw_std, l_weight=0.6, dbw_weight=0.4):
        """计算综合指标标准差的近似值"""
...
def main():
    ...
    try:
        # 交互式选择checkpoint目录
        selected_dir = _select_checkpoint_directory()
        if selected_dir is None:
            print("❌ 未选择checkpoint目录，程序退出")
            return
        
        # 在选择的目录中查找最新的checkpoint文件
        print(f"\n🔍 在目录 {os.path.basename(selected_dir)} 中查找最新的checkpoint...")
        ckpt = _find_latest_checkpoint_in_dir(selected_dir)
        print(f"📂 找到checkpoint文件: {os.path.basename(ckpt)}")
