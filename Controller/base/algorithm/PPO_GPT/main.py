...
def evaluate_performance(policy, env_config):
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            ...
            # 获取当前任务数
            num_tasks = state[0].num_nodes
            # 检查动作长度
            if not isinstance(action['task'], list):
                action['task'] = [action['task']]
            if len(action['task']) != num_tasks:
                raise ValueError(f"动作长度 {len(action['task'])} 与任务数 {num_tasks} 不一致，action['task']={action['task']}")
            # 同理，带宽动作也要对齐
            if not isinstance(action['bw'], list):
                action['bw'] = [action['bw']]
            if len(action['bw']) != len(state[0].edge_index[0]):
                # 这里 edge_index[0] 是边的数量，按需调整
                print(f"警告：带宽动作长度 {len(action['bw'])} 与任务链路数 {len(state[0].edge_index[0])} 不一致")
