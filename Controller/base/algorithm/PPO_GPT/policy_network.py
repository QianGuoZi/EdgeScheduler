...
class PolicyNetwork(nn.Module):
    ...
    def act(self, state, task_mask=None, path_mask=None, exploration=True):
        ...
        if task_mask is not None:
            ...
            # 检查每一行是否全为 -1e9
            if (task_logits == -1e9).all(dim=1).any():
                print("警告：有 task_logits 行被全部 mask！")
        ...
        # 获取任务链路数
        num_links = task_data.edge_index.shape[1]
        # 用同一个分布采样 num_links 个带宽动作
        bw_dist = Normal(bw_mean.squeeze(), bw_std.squeeze())
        bw_action = bw_dist.sample((num_links,))  # shape: [num_links]
        bw_action = bw_action.cpu().numpy().tolist()

        # 计算带宽动作的 log_prob
        bw_log_prob = bw_dist.log_prob(torch.tensor(bw_action, device=bw_mean.device))
        bw_log_prob = bw_log_prob.mean()  # 或 .sum()，看奖励设计
        
        # 保证 task_action 是一维 list
        if isinstance(task_action, torch.Tensor):
            task_action = task_action.cpu().numpy().tolist()
            # 如果 task_action 是标量，转为单元素 list
            if not isinstance(task_action, list):
        ...
        print("action['task']:", action['task'])
        print("num_tasks:", state[0].num_nodes)
        print("num_links:", num_links, "bw_action_len:", len(bw_action))
