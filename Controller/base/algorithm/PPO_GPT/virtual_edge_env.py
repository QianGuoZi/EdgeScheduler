...
class VirtualEdgeEnv:
    ...
    def create_resource_pool(self):
        """ 创建虚拟边缘集群 """
        # 创建节点
        print(f"Creating resource pool with {self.config['num_phys_nodes']} nodes")
        nodes = [
            {
                "id": i, 
        ...
        for i, node in enumerate(nodes):
            print(f"node {i}: cpu={node['cpu']}, ram={node['ram']}")
        
        # 创建链路
        links = []
        link_id = 0
        for i in range(self.config['num_phys_nodes']):
            for j in range(self.config['num_phys_nodes']):
                # 随机决定是否创建链路
                if random.random() > self.config['link_sparsity']:
                    ...
                    link_id += 1
                    print(f"link {link_id}: source={i}, target={j}, bandwidth={links[-1]['bandwidth']}")
    ...
    def generate_task_request(self):
        """ 生成虚拟任务请求 """
        print(f"Generating task request with {self.config['min_tasks']} to {self.config['max_tasks']} tasks")
        # 随机确定任务数量
        ...
        for i, task_node in enumerate(task_nodes):
            print(f"task_nodes {i}: cpu_req={task_node['cpu_req']}, ram_req={task_node['ram_req']}")
        ...
        for i in range(num_tasks):
            for j in range(num_tasks):
                ...
                if random.random() > 0.5:  # 50%的节点间有连接
                    ...
                    print(f"task_links {len(task_links)}: source={f't{i}'}, target={f't{j}'}, min_bw={min_bw}, max_bw={max_bw}")
