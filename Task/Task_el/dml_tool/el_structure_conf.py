...
def gen_conf (all_node, conf_json, link_json, output_path, args_taskid):
	...
	for src in link_json:
		...
		for link in link_list:
			...
			assert destName not in conf.connect, Exception (
				'duplicate link from ' + src + ' to ' + destName)
	...
	for name in node_conf_map:
		conf_path = os.path.join (output_path, args_taskid + '_' + name + '_structure.conf')
...
if __name__ == '__main__':
	...
	parser.add_argument ('-l', '--link', dest='link', required=False, type=str,
		default='links_range.json', help='./relative/path/to/link/json/file, default = links_range.json')
